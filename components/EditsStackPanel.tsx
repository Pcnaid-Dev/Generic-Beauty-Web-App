...
interface EditsStackPanelProps {
  layers: EditLayer[];
...
const EditsStackPanel: React.FC<EditsStackPanelProps> = ({ layers, onToggleVisibility, onSetOpacity, onReorder, onClose }) => {
  ...
  return (
    <div 
        className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-end animate-fade-in backdrop-blur-sm"
        onClick={onClose}
        aria-modal="true"
        role="dialog"
    >
      <div 
        className="bg-gray-800 border-l border-gray-700 w-full max-w-sm h-full flex flex-col shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-4 border-b border-gray-700 flex items-center justify-between flex-shrink-0">
            <h2 className="text-xl font-bold text-white">Edits Stack</h2>
            <button
                onClick={onClose}
            ...
            {reversedLayers.map((layer, index) => (
                ...
                >
                    <div className="flex items-center gap-3">
                        ...
                        >
                            {layer.isVisible ? <EyeIcon className="w-5 h-5" /> : <EyeOffIcon className="w-5 h-5" />}
                        </button>
                    </div>
                    {/* Opacity Slider */}
...
export default EditsStackPanel;